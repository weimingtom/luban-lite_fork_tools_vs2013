...
void lv_draw_mask_radius_init(lv_draw_mask_radius_param_t * param, const lv_area_t * rect, lv_coord_t radius, bool inv)
{
    ...
    if(!entry) {
		entry = (_lv_draw_mask_radius_circle_dsc_t *)lv_mem_alloc(sizeof(_lv_draw_mask_radius_circle_dsc_t));
...
void lv_draw_mask_polygon_init(lv_draw_mask_polygon_param_t * param, const lv_point_t * points, uint16_t point_cnt)
{
	...
	lv_point_t * p = (lv_point_t *)lv_mem_alloc(point_cnt * sizeof(lv_point_t));
    if(p == NULL) return;
    uint16_t i;
    uint16_t pcnt = 0;
    p[0] = points[0];
...
static void circ_calc_aa4(_lv_draw_mask_radius_circle_dsc_t * c, lv_coord_t radius)
{
    ...
    c->buf = (uint8_t *)lv_mem_alloc(radius * 6 + 6);  /*Use uint16_t for opa_start_on_y and x_start_on_y*/
	...
	lv_coord_t * cir_x = (lv_coord_t *)lv_mem_buf_get((radius + 1) * 2 * 2 * sizeof(lv_coord_t));
