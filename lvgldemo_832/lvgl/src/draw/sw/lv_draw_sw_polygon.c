                        ...
                        uint16_t point_cnt)
{
	...
	lv_point_t * p = (lv_point_t *)lv_mem_buf_get(point_cnt * sizeof(lv_point_t));
    if(p == NULL) return;
    uint16_t i;
    uint16_t pcnt = 0;
    p[0] = points[0];
	...
	lv_draw_mask_line_param_t * mp = (lv_draw_mask_line_param_t *)lv_mem_buf_get(sizeof(lv_draw_mask_line_param_t)* point_cnt);
