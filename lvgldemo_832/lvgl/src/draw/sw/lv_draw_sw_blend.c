    ...
    if(mask == NULL) {
        for(y = blend_area->y1; y <= blend_area->y2; y++) {
            for(x = blend_area->x1; x <= blend_area->x2; x++) {
				disp->driver->set_px_cb(disp->driver, (uint8_t *)(void *)dest_buf, dest_stride, x, y, color, opa);
            }
        }
    }
    else {
        ...
        for(y = 0; y < h; y++) {
            for(x = 0; x < w; x++) {
                if(mask[x]) {
					...
					disp->driver->set_px_cb(disp->driver, (uint8_t *)(void *)dest_buf, dest_stride, blend_area->x1 + x, blend_area->y1 + y, color,
...

{
    ...
    if(mask == NULL) {
        for(y = 0; y < h; y++) {
            for(x = 0; x < w; x++) {
				disp->driver->set_px_cb(disp->driver, (uint8_t *)(void *)dest_buf, dest_stride, dest_area->x1 + x, dest_area->y1 + y, src_buf[x],
                                        opa);
            }
            src_buf += src_stride;
        }
