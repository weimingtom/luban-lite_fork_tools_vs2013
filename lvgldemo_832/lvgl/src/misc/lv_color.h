...
#if LV_COLOR_16_SWAP == 0
//see upper
static lv_color_t LV_COLOR_MAKE16(uint8_t r8, uint8_t g8, uint8_t b8)
{
	lv_color_t result = { 0 };
	result.ch.blue = (uint8_t)((b8 >> 3) & 0x1FU);
	result.ch.green = (uint8_t)((g8 >> 2) & 0x3FU);
	result.ch.red = (uint8_t)((r8 >> 3) & 0x1FU);
	return result;
}
#endif
