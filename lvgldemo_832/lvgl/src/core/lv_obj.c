...
void lv_obj_allocate_spec_attr(lv_obj_t * obj)
{
    ...
    if(obj->spec_attr == NULL) {
        static uint32_t x = 0;
        x++;
		obj->spec_attr = (_lv_obj_spec_attr_t *)lv_mem_alloc(sizeof(_lv_obj_spec_attr_t));
...
#if LV_DRAW_COMPLEX
        if(clip_corner) {
			lv_draw_mask_radius_param_t * mp = (lv_draw_mask_radius_param_t *)lv_mem_buf_get(sizeof(lv_draw_mask_radius_param_t));
...
static void lv_obj_set_state(lv_obj_t * obj, lv_state_t new_state)
{
	...
	_lv_obj_style_transition_dsc_t * ts = (_lv_obj_style_transition_dsc_t *)lv_mem_buf_get(sizeof(_lv_obj_style_transition_dsc_t)* STYLE_TRANSITION_MAX);
