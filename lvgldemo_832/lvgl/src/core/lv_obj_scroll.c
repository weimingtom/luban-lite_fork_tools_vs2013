...
static void scroll_x_anim(void * obj, int32_t v)
{
	_lv_obj_scroll_by_raw((_lv_obj_t *)obj, v + lv_obj_get_scroll_x((const _lv_obj_t *)obj), 0);
}

static void scroll_y_anim(void * obj, int32_t v)
{
	_lv_obj_scroll_by_raw((_lv_obj_t *)obj, 0, v + lv_obj_get_scroll_y((const _lv_obj_t *)obj));
}

static void scroll_anim_ready_cb(lv_anim_t * a)
{
	lv_event_send((_lv_obj_t *)a->var, LV_EVENT_SCROLL_END, NULL);
}
