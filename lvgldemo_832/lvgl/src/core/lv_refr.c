...
void refr_obj(lv_draw_ctx_t * draw_ctx, lv_obj_t * obj)
{
    ...
    if(layer_type == LV_LAYER_TYPE_NONE) {
        ...
        if(_lv_area_is_in(&layer_area_full, &obj->coords, 0)) {
            ...
            lv_event_send(obj, LV_EVENT_COVER_CHECK, &info);
			if (info.res == LV_COVER_RES_COVER) flags = (lv_draw_layer_flags_t)(flags & ~LV_DRAW_LAYER_FLAG_HAS_ALPHA);
        }

		if (layer_type == LV_LAYER_TYPE_SIMPLE) flags = (lv_draw_layer_flags_t)(flags | LV_DRAW_LAYER_FLAG_CAN_SUBDIVIDE);
    ...
    if(drv->rotated == LV_DISP_ROT_180) {
        ...
        while(row < area_h) {
            ...
            if((row == 0) && (area_h >= area_w)) {
				...
				if (rot_buf == NULL) rot_buf = (lv_color_t *)lv_mem_buf_get(LV_DISP_ROT_MAX_BUF);
