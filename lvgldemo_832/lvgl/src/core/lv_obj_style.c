...
void lv_obj_add_style(lv_obj_t * obj, lv_style_t * style, lv_style_selector_t selector)
{
    ...
    obj->style_cnt++;
	obj->styles = (_lv_obj_style_t *)lv_mem_realloc(obj->styles, obj->style_cnt * sizeof(_lv_obj_style_t));
...
static lv_style_t * get_local_style(lv_obj_t * obj, lv_style_selector_t selector)
{
    ...
    lv_memset_00(&obj->styles[i], sizeof(_lv_obj_style_t));
	obj->styles[i].style = (lv_style_t *)lv_mem_alloc(sizeof(lv_style_t));
...
static _lv_obj_style_t * get_trans_style(lv_obj_t * obj,  lv_style_selector_t selector)
{
	...
	obj->styles[0].style = (lv_style_t *)lv_mem_alloc(sizeof(lv_style_t));
...
static void trans_anim_ready_cb(lv_anim_t * a)
{
    ...
    bool running = false;
    trans_t * tr_i;
	_LV_LL_READ(&LV_GC_ROOT(_lv_obj_style_trans_ll), tr_i, trans_t *) {
...
static void fade_in_anim_ready(lv_anim_t * a)
{
	lv_obj_remove_local_style_prop((_lv_obj_t *)a->var, LV_STYLE_OPA, 0);
}
