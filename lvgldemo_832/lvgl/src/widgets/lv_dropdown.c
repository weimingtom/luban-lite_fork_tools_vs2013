...
void lv_dropdown_set_options(lv_obj_t * obj, const char * options)
{
    ...
    dropdown->options = (char *)lv_mem_alloc(len);
...
void lv_dropdown_add_option(lv_obj_t * obj, const char * option, uint32_t pos)
{
	...
	dropdown->options = (char *)lv_mem_realloc(dropdown->options, new_len + 1);
    LV_ASSERT_MALLOC(dropdown->options);
    if(dropdown->options == NULL) return;
	...
	char * ins_buf = (char *)lv_mem_buf_get(ins_len + 2); /*+ 2 for terminating NULL and possible \n*/
...
static void draw_main(lv_event_t * e)
{
    ...
    if(dropdown->text) opt_txt = dropdown->text;
        ...
        char * buf = (char *)lv_mem_buf_get(128);
