...
void lv_table_set_cell_value(lv_obj_t * obj, uint16_t row, uint16_t col, const char * txt)
{
    ...
    table->cell_data[cell] = (char *)lv_mem_realloc(table->cell_data[cell], to_allocate);
...
#else
	table->cell_data[cell] = (char *)lv_mem_realloc(table->cell_data[cell], len + 2); /*+1: trailing '\0; +1: format byte*/
    LV_ASSERT_MALLOC(table->cell_data[cell]);
    if(table->cell_data[cell] == NULL) {
        va_end(ap2);
        return;
...
void lv_table_set_row_cnt(lv_obj_t * obj, uint16_t row_cnt)
{
    ...
    table->row_h = (lv_coord_t *)lv_mem_realloc(table->row_h, table->row_cnt * sizeof(table->row_h[0]));
    ...
    table->cell_data = (char **)lv_mem_realloc(table->cell_data, table->row_cnt * table->col_cnt * sizeof(char *));
...
void lv_table_set_col_cnt(lv_obj_t * obj, uint16_t col_cnt)
{
	...
	char ** new_cell_data = (char **)lv_mem_alloc(table->row_cnt * table->col_cnt * sizeof(char *));
    ...
    table->col_w = (lv_coord_t *)lv_mem_realloc(table->col_w, col_cnt * sizeof(table->col_w[0]));
...
void lv_table_add_cell_ctrl(lv_obj_t * obj, uint16_t row, uint16_t col, lv_table_cell_ctrl_t ctrl)
{
    ...
    if(is_cell_empty(table->cell_data[cell])) {
        table->cell_data[cell]    = (char *)lv_mem_alloc(2); /*+1: trailing '\0; +1: format byte*/
        LV_ASSERT_MALLOC(table->cell_data[cell]);
        if(table->cell_data[cell] == NULL) return;
...
static void lv_table_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    table->col_cnt = 1;
    table->row_cnt = 1;
    table->col_w = (lv_coord_t *)lv_mem_alloc(table->col_cnt * sizeof(table->col_w[0]));
