                         ...
                         int32_t pivot_x, int32_t pivot_y, bool antialias)
{
	...
	lv_color_t * cbuf = (lv_color_t *)lv_mem_alloc(dest_img->header.w * sizeof(lv_color_t));
	lv_opa_t * abuf = (lv_opa_t *)lv_mem_alloc(dest_img->header.w * sizeof(lv_opa_t));
...
void lv_canvas_blur_hor(lv_obj_t * obj, const lv_area_t * area, uint16_t r)
{
	...
	uint8_t * line_buf = (uint8_t *)lv_mem_buf_get(line_w);
...
void lv_canvas_blur_ver(lv_obj_t * obj, const lv_area_t * area, uint16_t r)
{
	...
	uint8_t * col_buf = (uint8_t *)lv_mem_buf_get(col_w);
    lv_img_dsc_t line_img;
...
static void init_fake_disp(lv_obj_t * canvas, lv_disp_t * disp, lv_disp_drv_t * drv, lv_area_t * clip_area)
{
	...
	lv_draw_ctx_t * draw_ctx = (lv_draw_ctx_t *)lv_mem_alloc(sizeof(lv_draw_sw_ctx_t));
