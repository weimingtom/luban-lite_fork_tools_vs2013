...
lv_obj_t * lv_tabview_add_tab(lv_obj_t * obj, const char * name)
{
    ...
    if(tabview->tab_pos & LV_DIR_VER) {
        new_map = (char **)lv_mem_alloc((tab_id + 1) * sizeof(const char *));
        lv_memcpy_small(new_map, old_map, sizeof(const char *) * (tab_id - 1));
        new_map[tab_id - 1] = (char *)lv_mem_alloc(strlen(name) + 1);
        strcpy((char *)new_map[tab_id - 1], name);
        new_map[tab_id] = "";
    }
    /*left or right dir*/
    else {
        new_map = (char **)lv_mem_alloc((tab_id * 2) * sizeof(const char *));
        lv_memcpy_small(new_map, old_map, sizeof(const char *) * (tab_id - 1) * 2);
        if(tabview->tab_cnt == 0) {
            new_map[0] = (char *)lv_mem_alloc(strlen(name) + 1);
            strcpy((char *)new_map[0], name);
            new_map[1] = "";
        }
        else {
            new_map[tab_id * 2 - 3] = "\n";
            new_map[tab_id * 2 - 2] = (char *)lv_mem_alloc(strlen(name) + 1);
            new_map[tab_id * 2 - 1] = "";
...
void lv_tabview_rename_tab(lv_obj_t * obj, uint32_t id, const char * new_name)
{
    ...
    tabview->map[id] = (char *)lv_mem_alloc(strlen(new_name) + 1);
...
static void lv_tabview_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    tabview->map = (char **)lv_mem_alloc(sizeof(const char *));
    tabview->map[0] = "";
