...
void lv_chart_set_type(lv_obj_t * obj, lv_chart_type_t type)
{
    ...
    if(chart->type == LV_CHART_TYPE_SCATTER) {
		...
		_LV_LL_READ_BACK(&chart->series_ll, ser, lv_chart_series_t *) {
    ...
    if(type == LV_CHART_TYPE_SCATTER) {
        lv_chart_series_t * ser;
		_LV_LL_READ_BACK(&chart->series_ll, ser, lv_chart_series_t *) {
            ser->x_points = (lv_coord_t *)lv_mem_alloc(sizeof(lv_point_t) * chart->point_cnt);
...
void lv_chart_get_point_pos_by_id(lv_obj_t * obj, lv_chart_series_t * ser, uint16_t id, lv_point_t * p_out)
{
    ...
    if(chart->type == LV_CHART_TYPE_LINE) {
		...
		_LV_LL_READ_BACK(&chart->series_ll, ser_i, lv_chart_series_t *) {
            if(ser_i == ser) break;
            p_out->x += col_w;
        }
...
lv_chart_series_t * lv_chart_add_series(lv_obj_t * obj, lv_color_t color, lv_chart_axis_t axis)
{
    ...
    ser->color  = color;
    ser->y_points = (lv_coord_t *)lv_mem_alloc(sizeof(lv_coord_t) * chart->point_cnt);
    LV_ASSERT_MALLOC(ser->y_points);

    if(chart->type == LV_CHART_TYPE_SCATTER) {
		ser->x_points = (lv_coord_t *)lv_mem_alloc(sizeof(lv_coord_t)* chart->point_cnt);
        LV_ASSERT_MALLOC(ser->x_points);
    }
...
static void draw_cursors(lv_obj_t * obj, lv_draw_ctx_t * draw_ctx)
{
	...
	_LV_LL_READ_BACK(&chart->cursor_ll, cursor, lv_chart_cursor_t *) {
...
static void new_points_alloc(lv_obj_t * obj, lv_chart_series_t * ser, uint32_t cnt, lv_coord_t ** a)
{
    ...
    if(ser->start_point != 0) {
		lv_coord_t * new_points = (lv_coord_t *)lv_mem_alloc(sizeof(lv_coord_t)* cnt);
        ...
        (*a) = (lv_coord_t *)lv_mem_realloc((*a), sizeof(lv_coord_t) * cnt);
        LV_ASSERT_MALLOC((*a));
        if((*a) == NULL) return;
