...
lv_obj_t * lv_menu_page_create(lv_obj_t * parent, char * title)
{
    ...
    if(title) {
        page->title = (char *)lv_mem_alloc(strlen(title) + 1);
...
void lv_menu_set_load_page_event(lv_obj_t * menu, lv_obj_t * obj, lv_obj_t * page)
{
	...
	lv_menu_load_page_event_data_t * event_data = (lv_menu_load_page_event_data_t *)lv_mem_alloc(sizeof(lv_menu_load_page_event_data_t));
