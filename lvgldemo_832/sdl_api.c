...
#include "lv_conf.h" //for LV_HOR_RES
...
#else
	width = MY_DISP_HOR_RES;// 480;// 320; //LV_HOR_RES
	height = MY_DISP_VER_RES;// 320;// 240; //LV_VER_RES
	MainFrameResize(width, height);
	//Nofrendo_main_entry();
	main();
#endif
...
void CanvasDrawPixel(int x1, int y1, int color, int alpha)
{
	if (alpha != 255)
	{
		//FIXME:
		//__asm int 3;
		//SetPixel(s_hBackBufferDC, x1, y1, (COLORREF)color);
	}
	else
	{
#if 0
